...
async def get_current_members(
    ...
    if as_of_date is None:
        # Get latest as_of_date for this index
...
async def get_all_current_tickers(db: AsyncSession) -> list[str]:
    """Union of all index members at their latest as_of_date."""
    indices_result = await db.execute(select(Index))
    indices = list(indices_result.scalars().all())
    ...
    for idx in indices:
        ...
        all_tickers.update(row[0] for row in result.all())

    return sorted(all_tickers)


async def get_ticker_indices(db: AsyncSession, ticker: str) -> list[str]:
    """Which indices a ticker belongs to (at latest membership date)."""
    indices_result = await db.execute(select(Index))
    indices = list(indices_result.scalars().all())
    ...
    for idx in indices:
        ...
        if exists.scalar_one_or_none() is not None:
            result.append(idx.short_name)

    return result


async def build_indices_map(db: AsyncSession) -> dict[str, list[str]]:
    """Build {ticker: [index_short_names]} for all current members."""
    indices_result = await db.execute(select(Index))
    indices = list(indices_result.scalars().all())
    ...
    for idx in indices:
        ...
        for (ticker,) in members.all():
            ticker_map.setdefault(ticker, []).append(idx.short_name)
