...
import { getActiveTickers, filterMultiples } from '../lib/filters';
...
export default function MultiplesChart({ data, state, dispatch }: MultiplesChartProps) {
  ...
  const { avgs, q75s } = useMemo(() => {
    ...
    for (let di = 0; di < data.dates.length; di++) {
      const vals = filterMultiples(data, type, di, activeTickers, state.epsCap);
      if (vals.length < 4) {
        avgs.push(null);
        q75s.push(null);
        continue;
  ...
  const datasets = useMemo(() => {
    ...
    return ds;
  }, [avgs, q75s, state.hlTk, data, mk, col]);
